...
export default function JsonBulkImporter() {
  ...
  const validateJson = (): boolean => {
    try {
      if (!jsonContent.trim()) {
        addProgress("⚠️ No JSON content provided");
        return false;
      }
      ...
      if (!data.subjects || !Array.isArray(data.subjects)) {
        addProgress("⚠️ Invalid JSON structure: Missing 'subjects' array");
        return false;
      }
      ...
      for (const subject of data.subjects) {
        if (!subject.name || !subject.code) {
          addProgress("⚠️ Invalid subject: Missing name or code");
          return false;
        }

        for (const module of subject.modules || []) {
          if (!module.name) {
            addProgress(`⚠️ Invalid module in ${subject.name}: Missing name`);
            return false;
          }

          for (const topic of module.topics || []) {
            if (!topic.name) {
              addProgress(`⚠️ Invalid topic in ${module.name}: Missing name`);
              return false;
            }

            for (const skill of topic.skills || []) {
              if (!skill.name) {
                addProgress(`⚠️ Invalid skill in ${topic.name}: Missing name`);
                return false;
              }
              ...
              for (const question of skill.questions || []) {
                if (!question.question_text || !question.correct_answer) {
                  addProgress(
                    `⚠️ Invalid question in ${skill.name}: Missing text or correct answer`
                  );
                  return false;
                }
                if (
                  ...
                  addProgress(
                    `⚠️ Invalid question in ${skill.name}: Need at least 2 options`
                  );
                  return false;
                }
              }
      ...
      setParsedData(data);
      addProgress(
        `✅ Validated: ${data.subjects.length} subjects, ${
  ...
  const importContent = async () => {
    ...
    try {
      ...
      for (const subject of parsedData.subjects) {
        ...
        totalSubjects++;
        addProgress(`  ✅ Subject: ${subject.name} (ID: ${subjectId})`);
        ...
        for (const module of subject.modules || []) {
          ...
          totalModules++;
          addProgress(`    ✅ Module: ${module.name} (ID: ${moduleId})`);

          for (const topic of module.topics || []) {
            addProgress(`    🗂️ Creating topic: ${topic.name}`);

            const topicId = await createTopic(moduleId, topic);
            totalTopics++;
            addProgress(`      ✅ Topic: ${topic.name} (ID: ${topicId})`);
            ...
            for (const skill of topic.skills || []) {
              ...
              totalSkills++;
              addProgress(`        ✅ Skill: ${skill.name} (ID: ${skillId})`);

              for (const question of skill.questions || []) {
                addProgress(`        ❓ Question for: ${skill.name}`);

                await createQuestion(skillId, question);
                totalQuestions++;
              }
              addProgress(
                `        ✅ ${skill.questions?.length || 0} questions for ${
                  skill.name
                }`
              );
            }
      ...
      addProgress(
        `📊 Summary: ${totalSubjects} subjects, ${totalModules} modules, ${totalTopics} topics, ${totalSkills} skills, ${totalQuestions} questions`
      );
  ...
  const loadSampleJson = () => {
    ...
    addProgress("ℹ️ Sample JSON loaded. Click 'Validate JSON' to check structure.");
  };
            ...
            <div className="bg-gray-900 text-green-400 font-mono text-sm p-4 rounded-lg h-96 overflow-y-auto">
              {importProgress.length === 0 ? (
                <div className="text-gray-400">
                  ...
                  <br />
                  • Each question: question_text, question_data.options[], correct_answer
                  <br />
                  • Optional: question_data.image_url, question_data.mermaid, question_data.diagram
                  <br />
